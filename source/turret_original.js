window.V016Turret=(()=>{
  ...
  const starter=()=>({id:'hmg016_1',ammo:150,angle:-Math.PI*3/4,enabled:true,x:1410,y:1050,wallId:'v091wall015_SR_3',fallen:false});
  guns.push(starter());
  ...
  function fits(p,w){return !!w&&[p.x,p.y].every(Number.isFinite)&&p.x>=w.x+Math.min(12,w.w/2)&&p.x<=w.x+w.w-Math.min(12,w.w/2)&&p.y>=w.y+Math.min(12,w.h/2)&&p.y<=w.y+w.h-Math.min(12,w.h/2);}
  function support(t){return !t.fallen&&eligible(V015Base.byId.get(t.wallId))&&fits(t,V015Base.byId.get(t.wallId));}
  ...
  function candidate(x,y){
    ...
    for(const w of V015Base.sections){if(!eligible(w))continue;
      const px=Math.min(12,w.w/2),py=Math.min(12,w.h/2),p={x:clamp(x,w.x+px,w.x+w.w-px),y:clamp(y,w.y+py,w.y+w.h-py),wallId:w.id};
      const d=distance(x,y,p.x,p.y);if(d<bestD&&d<=76){best=p;bestD=d;}
    }return best;
  }
  ...
  const tapOld=V0105.tapWorld;V0105.tapWorld=function(x,y){if(placement)return selectPoint(x,y);if(scene==='surface'&&V091Fortress.corners.some(c=>{const p=player.wallLevel?c:c.foot;return distance(x,y,p.x,p.y)<=13;}))return false;const t=scene==='surface'&&guns.find(t=>distance(x,y,t.x,t.y)<28);if(t){approachObject({id:t.id,kind:TYPE});return true;}return tapOld(x,y);};
  ...
  const restoreOld=restoreGameProgress;restoreGameProgress=function(d){validate(d.turret016,d);restoreOld(d);guns.splice(0,guns.length,...copy(d.turret016?.guns||[starter()]));nextId=d.turret016?.nextId||2;runtime.clear();selected=null;cancelPlacement();obstacleRevision=-1;settleUnsupported();};
