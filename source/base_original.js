window.V015Base=(()=>{
  ...
  // Eight panels on each outer side. No overlapping invisible wall remains.
  for(const [side,y] of [['N',130],['S',998]]){
    span('v091wall015_'+side+'L',170,y,560,72,4,OUTER_HP,'outer',side);
    span('v091wall015_'+side+'R',870,y,560,72,4,OUTER_HP,'outer',side);
  }
  span('v091wall015_W',170,202,72,796,8,OUTER_HP,'outer','W');
  span('v091wall015_E',1358,202,72,796,8,OUTER_HP,'outer','E');
  add('gate',gateRect.x,gateRect.y,gateRect.w,gateRect.h,OUTER_HP,'gate','S','south');
  add('v015northGate',730,130,140,72,OUTER_HP,'gate','N','north');
  add('v091innerGate',730,1170,140,20,INNER_HP,'gate','S','airlock');
  span('v015airlockW',714,1070,16,100,3,INNER_HP,'airlock','W');
  span('v015airlockE',870,1070,16,100,3,INNER_HP,'airlock','E');
  ...
  function deckPresent(x,y,snapshot){
    ...
    // The old south gate has a bridge over the ground-level opening.
    if(x>=730&&x<=870&&y>=998&&y<=1070)return hp(byId.get('gate'))>0;
    return sections.some(o=>(o.group==='outer'||o.gate==='north')&&hp(o)>0&&x>=o.x&&x<=o.x+o.w&&y>=o.y&&y<=o.y+o.h);
  }
  ...
  function damage(id,amount){
    ...
    if(o.hp===0){changed();if(scene==='surface'&&player.wallLevel&&!V091Fortress.transitioning&&V091Fortress.elevatedCollision(player.x,player.y,player.radius)){player.wallLevel=false;V091Fortress.cancelRoute();settle(player);}
  ...
  interactionObjects=function(which=scene){const old=oldInteractions(which);if(which!=='surface')return old;
    return [...old.filter(o=>o.id!=='yard_generator'&&!o.id?.startsWith('v015prop_')&&!(byId.has(o.id)&&byId.get(o.id).hp===0)),...sections.filter(o=>o.gate&&!['south','airlock'].includes(o.gate)&&o.hp>0).map(o=>({...o,kind:'baseGate015',name:isOpen(o)?'Закрыть ворота':'Открыть ворота',ref:o.id,range:64}))];};
  ...
  executeInteraction=function(o){if(o?.kind==='baseGate015'){if(canInteract(o,player.x,player.y))toggle(byId.get(o.ref));return;}
    ...
    return oldExecute(o);};
  function capture(){return{schema:3,northOpen,commandNorthOpen,commandSouthOpen,sections:sections.map(o=>({id:o.id,hp:o.hp,level:o.level}))};}
  function validate(d){if(!d)return true;
    const legacy=d.schema===1,expected=sections.length+(legacy?retiredSections.size:0);
    if((!legacy&&d.schema!==2&&d.schema!==3)||!['northOpen','commandNorthOpen','commandSouthOpen'].every(k=>typeof d[k]==='boolean')||!Array.isArray(d.sections)||d.sections.length!==expected)throw Error('Некорректное состояние укреплений');
    const seen=new Set();for(const p of d.sections){const section=byId.get(p?.id),maxHp=d.schema===3?(section&&Number.isInteger(p.level)&&p.level>=1&&p.level<=5?[0,10000,20000,40000,60000,100000][p.level]:undefined):(section?.legacyMaxHp??(legacy?retiredSections.get(p?.id):undefined));if(maxHp===undefined||seen.has(p.id)||!Number.isFinite(p.hp)||p.hp<0||p.hp>maxHp)throw Error('Некорректная прочность секции');seen.add(p.id);}return true;}
  function restore(d){validate(d);const values=new Map(d?.sections.map(o=>[o.id,o]));for(const o of sections){const p=values.get(o.id);o.level=d?.schema===3&&p?p.level:1;o.maxHp=[0,10000,20000,40000,60000,100000][o.level];o.hp=p?(d.schema===3?p.hp:Math.round(p.hp/o.legacyMaxHp*o.maxHp)):o.maxHp;o.hitAt=-1e6;}northOpen=d?.northOpen??false;commandNorthOpen=d?.commandNorthOpen??false;commandSouthOpen=d?.commandSouthOpen??true;revision++;invalidateGeometry();}
  const captureOld=captureGameProgress;captureGameProgress=function(){const d=captureOld();d.base015=capture();return d;};
  const decodeOld=decodeGameProgress;decodeGameProgress=function(raw){const probe=JSON.parse(raw);validate(probe.base015);validating=true;let d;try{d=decodeOld(raw);}finally{validating=false;}
    ...
    return d;};
  const restoreOld=restoreGameProgress;restoreGameProgress=function(d){restore(d.base015);restoreOld(d);
  ...
  const api={sections,rooms,props,byId,walls,isOpen,stage,damage,changed,toggle,deckPresent,rayEntry,blocker,siege,capture,validate,restore,freePoint,drawGround,drawFortifications,drawMap,inWorkshop:()=>false,get revision(){return revision;},get validating(){return validating;}};
  return api;

  function drawGround(){drawBaseFloor();drawProps();}
  function drawFortifications(){drawWalls();}
  function drawMap(c){c.fillStyle='#4d5956';c.fillRect(170,130,1260,940);for(const r of rooms){c.fillStyle='#778276';c.fillRect(r.x,r.y,r.w,r.h);}for(const o of walls()){c.fillStyle=o.hp/o.maxHp>.5?'#c0c7b1':'#bb8d62';c.fillRect(o.x,o.y,o.w,o.h);}c.fillStyle='#182a2a';c.fillRect(765,555,70,70);}
  ...
  function pad(x,y,r=32){if(!deckPresent(x,y)||!sections.some(o=>o.hp>0&&o.group==='outer'&&x>=o.x&&x<=o.x+o.w&&y>=o.y&&y<=o.y+o.h))return;
    metal(ctx,x-r,y-r,r*2,r*2,true);metal(ctx,x-r+4,y-r+4,r*2-8,r*2-8);rect(ctx,x-r+10,y-r+10,r*2-20,r*2-20,'#485a55','#263c3f',2);bolts(ctx,x-r,y-r,r*2,r*2);
    ctx.strokeStyle='#b3b79c55';ctx.lineWidth=1;ctx.setLineDash([4,4]);ctx.strokeRect(x-14,y-14,28,28);ctx.setLineDash([]);
    // Empty mounting sockets for a future player-built turret.
    for(const dx of [-11,11])for(const dy of [-11,11])disk(ctx,x+dx,y+dy,1.5,'#212e31');
  }
  function drawWalls(){ctx.save();
    gridFloor(ctx,730,1070,140,100,28,['#59625d','#606861']);
    for(const o of sections){if(!visibleOnScreen(o.x+o.w/2,o.y+o.h/2,Math.max(o.w,o.h)+35))continue;if(o.gate)gateArt(o);else ctx.drawImage(wallSprite(o),o.x-3,o.y-3);}
    for(const t of V091Fortress.corners){if(!deckPresent(t.x,t.y))continue;
      const dx=t.foot.x-t.x,dy=t.foot.y-t.y,a=Math.atan2(dy,dx),len=Math.hypot(dx,dy);ctx.save();ctx.translate(t.x,t.y);ctx.rotate(a);rect(ctx,21,-16,len-21,32,'#27383b','#8a9985');for(let p=25;p<len-4;p+=7){line(ctx,[[p,-13],[p,13]],'#8a9787',3);line(ctx,[[p+2,-13],[p+2,13]],'#111f2955',2);}line(ctx,[[23,-18],[len,-18]],'#adb5a1',2);line(ctx,[[23,18],[len,18]],'#adb5a1',2);ctx.restore();pad(t.x,t.y,34);}
    for(const y of [401,600,799]){pad(206,y,34);pad(1394,y,34);}for(const x of [450,1150]){pad(x,166,32);pad(x,1034,32);}
    drawLights(true);
