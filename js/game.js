...
const V09Craft = (() => {
 ...
 concrete:{station:'furnace',category:'Строительство',name:'Бетон',input:{stone:2},output:'concrete',qty:1,ms:2000},
 fishing_rod:{station:'craft_bench',category:'Инструменты',name:'Удочка',input:{wood:10,parts:2},output:'fishing_rod',qty:1,ms:4000},
    ...
    iron: {station:'furnace',category:'Металлы',name:'Железо',input:{iron_ore:1},output:'iron',qty:1,ms:2000},
    copper: {station:'furnace',category:'Металлы',name:'Медь',input:{copper_ore:1},output:'copper',qty:1,ms:2000},
  ...
  function makeJob(recipe,batches){const r=RECIPES[recipe];return {recipe,batches,totalMs:r.ms*batches,remainingMs:r.ms*batches,completedBatches:0,outputQty:0,collectedQty:0};}
  ...
  function collectType(id,type,limit){
    ...
    const stack=(ITEM[type].equip||ITEM[type].hand)?1:STACK_MAX;
  ...
  function cancelQueued(id,index){
    ...
    const j=queueExtra[id].splice(index,1)[0];reserveRefund(id,RECIPES[j.recipe],j.batches);collectRefund(id);queueGameSave();render();return true;
  }
  function cancelUnstarted(id){
    ...
    if(!cancelled)return 0;reserveRefund(id,RECIPES[j.recipe],cancelled);
  ...
  function normalizeSave(data){
    ...
    function normalJob(src,id,legacy){
      ...
      if(!r||r.station!==id||!int(j.batches,1,legacy?100:MAX_BATCHES)||j.totalMs!==r.ms*j.batches||!num(j.remainingMs,0,j.totalMs))fail();
      if(legacy){
  ...
  function queueValidate(data){
    ...
    for(const id of STATIONS){
      ...
      for(const j of list){const r=RECIPES[j?.recipe];if(!r||r.station!==id||!Number.isInteger(j.batches)||j.batches<1||j.batches>MAX_BATCHES||j.totalMs!==r.ms*j.batches||j.remainingMs!==j.totalMs||j.completedBatches!==0||j.outputQty!==0||j.collectedQty!==0)fail();}
...
window.V010Combat=(() => {
  ...
  renderEquipment=function(){refreshStats();oldRenderEquipment();const st=refreshStats(),node=el('characterStats');if(node)node.innerHTML='HP <b>'+Math.round(player.health)+'/'+Math.round(st.hp)+'</b> · Защита <b>'+st.armor+'%</b><br>Скорость <b>+'+Math.round(st.speed*100)+'%</b> · Точность <b>+'+Math.round(st.accuracy*100)+'%</b><br>Рюкзак: '+BAG_SLOTS+' мест';};
  ...
  updateAmmoHud=function(){
    ...
    const progress=reloading?' · '+(reloading.remainingMs/1000).toFixed(1)+' с':'';
    if(hud&&text+progress!==lastHud){hud.textContent=text+progress;lastHud=text+progress;}
    if(b){b.textContent=reloading?'↻ '+(reloading.remainingMs/1000).toFixed(1):'↻';b.style.setProperty('--reload-progress',reloading?Math.round(100*(1-reloading.remainingMs/reloading.totalMs))+'%':'0%');}
  ...
  document.addEventListener('keydown',e=>{if(e.key?.toLowerCase()==='r'&&!['INPUT','TEXTAREA','SELECT'].includes(e.target?.tagName)){e.preventDefault();reloadWeapon();}});
  const reloadButton=document.createElement('button');reloadButton.id='v010ReloadButton';reloadButton.type='button';reloadButton.title='Перезарядка · R';reloadButton.setAttribute('aria-label','Перезарядить оружие');reloadButton.addEventListener('pointerdown',e=>e.stopPropagation());reloadButton.addEventListener('click',reloadWeapon);document.body.append(reloadButton);
  const stopButton=document.createElement('button');stopButton.id='v010PracticeStop';stopButton.textContent='Завершить тренировку';stopButton.addEventListener('click',()=>setPractice(false));document.body.append(stopButton);
...
const V010Craft=(()=>{
  ...
  function refreshPin(){
    ...
    const html=`<b>${r.name} × ${r.qty*p.batches}</b>${rows}`;if(last!==html){pin.innerHTML=html;const x=v09Button('×',()=>{api.setPin(null);refreshPin();});x.title='Открепить рецепт';x.setAttribute('aria-label',x.title);pin.append(x);last=html;}
  }
...
const V010Energy=(()=>{
  ...
  function switchState(b,on,label){
    if(!b)return;b.textContent='';b.classList.toggle('on',on);b.setAttribute('role','switch');b.setAttribute('aria-checked',String(on));b.setAttribute('aria-pressed',String(on));b.setAttribute('aria-label',label+': '+(on?'включено':'выключено'));b.title=label+': '+(on?'включено':'выключено');
  }
  v09UpdateDeviceRow=function(row){
    const d=V09Power.devices[row.dataset.powerDevice];if(!d)return;
    const small=row.querySelector('small');if(small)small.textContent=v09DeviceStatus(d)+' · '+fmt(d.watts)+' кВт';
    switchState(row.querySelector('button'),d.enabled,'Питание: '+d.name);
  };
  function prioritySelect(value,label,callback){
    const select=document.createElement('select');select.className='v010Priority';select.setAttribute('aria-label',label);select.title=label;
    for(const n of [3,2,1]){const opt=document.createElement('option');opt.value=String(n);opt.textContent=priorityName(n);select.appendChild(opt);}select.value=String(value);select.onchange=()=>callback(+select.value);return select;
  }
  ...
  v09RenderCircuit=function(){
    ...
    const header=parent.querySelector('.v09CircuitHeader');if(header){const wrap=document.createElement('label');wrap.className='v010PriorityLabel';wrap.textContent='Приоритет комнаты';wrap.appendChild(prioritySelect(roomRank(room),'Приоритет: '+V09Power.rooms[room],n=>setRoomPriority(room,n)));header.appendChild(wrap);}
    ...
    const note=document.createElement('p');note.className='v09PowerNote';note.textContent='Сначала питаются комнаты с высоким приоритетом, затем приборы внутри них. Если энергии не хватает, остальные ждут и возобновляют работу автоматически.';parent.appendChild(note);
  };
  ...
  v09RefreshPowerUI=function(){
    ...
    document.querySelectorAll('[data-power]').forEach(node=>{
      const key=node.dataset.power;
      if(key==='running')node.textContent=V09Power.running?(a.load+a.chargeInput>0?'Работает':'Ожидание'):'Выключен';
      if(key==='fuel')node.textContent=V09Power.fuel.toFixed(1)+' / '+V09Power.capacity;
      if(key==='load')node.textContent=fmt(a.load)+' / '+fmt(a.supply)+' кВт';
      if(key==='fuelbar'||key==='tankbar')node.style[key==='tankbar'?'height':'width']=V09Power.fuel/V09Power.capacity*100+'%';
      if(key==='tankvalue')node.textContent=V09Power.fuel.toFixed(1)+' / '+V09Power.capacity;
      if(key==='bagfuel')node.textContent='В рюкзаке: '+bagCount('fuel')+' топлива';
      if(key==='generatorToggle')node.textContent=V09Power.running?'Остановить генератор':'Запустить генератор';
      if(key==='battery')node.textContent=Math.round(battery.charge/battery.capacity*100)+'% · '+fmt(battery.charge)+' кВт·ч';
      if(key==='batterybar')node.style.width=battery.charge/battery.capacity*100+'%';
      if(key==='reserve')node.textContent=remainingTime(a);
      if(key==='battery-flow')node.textContent=a.chargeInput>0?'Зарядка: '+fmt(a.chargeInput)+' кВт':a.batteryOutput>0?'Питание базы: '+fmt(a.batteryOutput)+' кВт':remainingTime(a);
      if(key==='battery-toggle')switchState(node,battery.enabled,'Резервная батарея');
      if(key==='warning')node.textContent=a.shed.length?'Приборов ожидают мощности: '+a.shed.length+'. Высокие приоритеты получают питание первыми.':a.supply<=0?'Нет энергии. Запустите генератор или подключите заряженную батарею.':a.batteryOutput>0?'Батарея питает базу · '+remainingTime(a):a.chargeInput>0?'Свободная мощность заряжает батарею: '+fmt(a.chargeInput)+' кВт':'Свободно '+fmt(a.supply-a.load)+' кВт';
    });
    document.querySelectorAll('[data-power-device]').forEach(v09UpdateDeviceRow);
    ...
    document.querySelectorAll('[data-room-power-status]').forEach(node=>{const room=node.dataset.roomPowerStatus;const devices=Object.values(V09Power.devices).filter(d=>d.room===room&&d.enabled&&d.active());const demand=devices.reduce((n,d)=>n+d.watts,0),load=devices.filter(d=>a.served.has(d.id)).reduce((n,d)=>n+d.watts,0);node.textContent=(V09Power.roomEnabled[room]?'Контур включён':'Контур отключён')+' · '+fmt(load)+' / '+fmt(demand)+' кВт';});
    document.querySelectorAll('[data-power-room]').forEach(node=>{
      ...
      node.classList.toggle('powered',powered);node.classList.toggle('waiting',enabled&&!powered);node.classList.toggle('selected',room===V09Power.selectedRoom);
      const small=node.querySelector('small');if(small)small.textContent=room==='corridor'?'':!enabled?'Выкл.':!powered?'Ожидание':fmt(devices.reduce((s,d)=>s+d.watts,0))+' кВт';
    });
    document.querySelectorAll('[data-power-spot]').forEach(node=>{const d=V09Power.devices[node.dataset.powerSpot];node.classList.toggle('powered',a.served.has(d.id));node.classList.toggle('waiting',d.enabled&&V09Power.roomEnabled.yard&&!a.served.has(d.id));});
  };
  function openBattery(){
    const overlay=v09Overlay('v010BatteryOverlay','Энергоблок · резервная батарея'),body=overlay.querySelector('.v09Body');body.innerHTML=v09PowerStats();
    const row=document.createElement('div');row.className='v09DeviceRow';const text=document.createElement('div');text.innerHTML='<strong>Резервное питание</strong><small data-power="battery-flow"></small>';row.appendChild(text);const button=v09Button('',()=>{battery.enabled=!battery.enabled;v09PowerChanged();},'v09DeviceToggle');button.dataset.power='battery-toggle';row.appendChild(button);body.appendChild(row);
    const note=document.createElement('p');note.className='v09PowerNote';note.textContent='Ёмкость: 1,5 кВт·ч. Зарядка до 3 кВт от свободной мощности генератора, отдача до 6 кВт. Батарея поддерживает базу при остановке генератора и помогает при большой нагрузке. Выключатель энергоблока не отключает резерв.';body.appendChild(note);
  ...
  v09OpenGenerator=function(refuel=false){originalGenerator(refuel);const body=el('v09GeneratorOverlay')?.querySelector('.v09Body');if(!body)return;const note=[...body.querySelectorAll('.v09PowerNote')].find(n=>n.textContent.includes('Мощность:'));if(note)note.textContent='Мощность генератора: '+fmt(V09Power.supply)+' кВт. Расход топлива зависит от выбранной сложности. Свободная мощность заряжает батарею; готовые изделия и прогресс сохраняются при остановке.';body.appendChild(v09Button('Резервная батарея',openBattery));};
  ...
  v09DrawEnergyReadouts=function(){
    originalReadouts();const a=allocation();ctx.save();ctx.textAlign='center';ctx.fillStyle='#46504b';ctx.fillRect(1050,431,154,48);ctx.fillStyle='#d8ecd4';ctx.font='12px Arial';ctx.fillText(fmt(a.load)+' / '+fmt(a.supply)+' кВт',1127,450);ctx.font='8px Arial';ctx.fillText(a.batteryOutput>0?'СЕТЬ + РЕЗЕРВ':a.chargeInput>0?'СЕТЬ · ЗАРЯДКА':'ПИТАНИЕ БАЗЫ',1127,468);ctx.fillStyle='#263a3c';ctx.fillRect(1253,340,69,105);ctx.strokeStyle='#8eada8';ctx.lineWidth=2;ctx.strokeRect(1264,361,47,61);ctx.fillStyle=battery.enabled?'#75b99b':'#667775';const fill=clamp(battery.charge/battery.capacity,0,1);ctx.fillRect(1268,418-53*fill,39,53*fill);ctx.fillStyle='#e1f0e8';ctx.font='12px Arial';ctx.fillText(Math.round(fill*100)+'%',1287,351);ctx.fillStyle='#24383c';ctx.fillRect(1239,479,97,27);ctx.fillStyle='#b5d4c8';ctx.font='9px Arial';ctx.fillText(!battery.enabled?'ОТКЛЮЧЕНА':a.batteryOutput>0?'ПИТАЕТ БАЗУ':a.chargeInput>0?'ЗАРЯЖАЕТСЯ':fill>0?'РЕЗЕРВ':'РАЗРЯЖЕНА',1287,493);ctx.restore();
  };
  v09Style(`
...
const V010Camera=(()=>{
  ...
  function updateSettings(){mini.style.opacity=String(OPACITY[opacity]);mini.style.setProperty('left',corner==='left'?'14px':'auto','important');mini.style.setProperty('right',corner==='left'?'auto':'14px','important');el('v010Zoom').value=Math.round(targetZoom*100);el('v010ViewMode').textContent=tilt===1?'Вид сверху':'Наклонный вид';el('v010MapCorner').textContent=corner==='left'?'Карта слева':'Карта справа';}
  ...
  mini.title='Касание: большая карта · удержание 1,5 с: 100% → 60% → 30%';
  ...
  window.addEventListener('pointerdown',e=>{if(e.pointerType!=='touch'||!worldInput(e)||menuOpen)return;if(joystickAt(e.clientX,e.clientY)||leftPointerId!==null||rightPointerId!==null)return;touches.set(e.pointerId,{x:e.clientX,y:e.clientY});if(touches.size===2){const [a,b]=[...touches.values()];pinch={distance:Math.hypot(a.x-b.x,a.y-b.y),zoom};stopControls(true);e.preventDefault();e.stopImmediatePropagation();}},{passive:false,capture:true});
  ...
  function mapDraw(c,w,h,miniMode){
    ...
    if(miniMode){c.fillStyle='#d6e2d6';c.font='9px Arial';c.textAlign='center';c.fillText(scene==='surface'?'N ↑':'Бункер',w/2,15);if(goal?.scene===scene){const dx=(goal.x-player.x)*scale,dy=(goal.y-player.y)*scale,d=Math.hypot(dx,dy),k=Math.min(1,(Math.min(w,h)/2-16)/Math.max(1,d));if(k<1){const x=w/2+dx*k,y=h/2+dy*k,a=Math.atan2(dy,dx);c.save();c.translate(x,y);c.rotate(a);c.fillStyle='#ebc87a';c.beginPath();c.moveTo(7,0);c.lineTo(-4,-4);c.lineTo(-4,4);c.closePath();c.fill();c.restore();}c.textAlign='center';c.fillText(Math.round(d/scale)+' м',w/2,h-11);}}
...
window.V013City=(()=>{
 ...
 function drawPlaces(){
  ...
  if(V011World.inside(mall,player.x,player.y,0)){ctx.save();if(floor){ctx.beginPath();ctx.rect(camera.x,camera.y,V010Camera.view().w,V010Camera.view().h);ctx.rect(mall.x,mall.y,mall.w,mall.h);ctx.clip('evenodd');ctx.fillStyle='#101e26ed';ctx.fillRect(camera.x,camera.y,V010Camera.view().w,V010Camera.view().h);ctx.restore();ctx.save();}
   for(const w of partitions()){ctx.fillStyle='#aec0b7';ctx.fillRect(w.x,w.y,w.w,w.h);}ctx.fillStyle='#adc3b6';ctx.font='13px sans-serif';ctx.textAlign='center';for(let i=0;i<5;i++)ctx.fillText(shops[floor*5+i][0],mall.x+i*208+100,mall.y+150);ctx.fillStyle='#6a827b';ctx.fillRect(stair.x,stair.y,stair.w,stair.h);ctx.strokeStyle='#cfdbcf';for(let j=0;j<7;j++){ctx.beginPath();ctx.moveTo(stair.x,stair.y+j*9);ctx.lineTo(stair.x+stair.w,stair.y+j*9);ctx.stroke();}ctx.fillStyle='#eff4db';ctx.fillText(floor?'↓ 1 этаж':'↑ 2 этаж',stair.x+35,stair.y-12);ctx.restore();}
 }
...
window.V015Base=(()=>{
  ...
  function toggle(o){if(!o?.gate||o.hp<=0)return false;if(o.gate==='south'){toggleGate();return true;}if(o.gate==='airlock'){V091Fortress.toggleInnerGate();return true;}
    ...
    if(o.gate==='north')northOpen=!northOpen;else if(o.gate==='commandN')commandNorthOpen=!commandNorthOpen;else commandSouthOpen=!commandSouthOpen;changed();updateAction();return true;
  }
...
window.V017Monsters=(()=>{
  ...
  drawZombie=function(z){
    ...
    const attack=r.attack>now||r.jump||r.fuse,frame=attack?4+Math.min(3,Math.max(0,Math.floor((r.jump?(now-r.jump.start)/r.jump.duration:r.fuse?(now-(r.fuseAt??now))/(r.fuseDuration||850):1-(r.attack-now)/400)*4))):Math.floor(r.walk/9)%4;
