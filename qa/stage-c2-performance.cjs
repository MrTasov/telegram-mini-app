// Immutable delivered 0.33.0 vs current Stage C2. Run separately from regression suites.
...
(async()=>{
 ...
 const checkpoint='qa/results/stage-c2-performance-progress.json';
 const retained=process.env.STAGE_C2_PERF_RESUME?JSON.parse(fs.readFileSync(checkpoint)):null;
 if(retained&&retained.runtimeSha256!==runtimeSha256)throw Error('Stale performance checkpoint');
 const pair=[boot(path.join(__dirname,'stage-c2-base')),boot(root)],rows=retained?.rows||[];
 ...
 for(const [width,height,mode]of (process.env.STAGE_C2_PREREQ?[[390,844,'MOBILE']]:[[390,844,'MOBILE'],[1280,800,'PC']]))for(const sceneName of (process.env.STAGE_C2_PREREQ?['bunker_moving_lights','night_combined_lights','bunker_door_spill']:['quiet_surface','stress_surface','bunker_hall','bunker_moving_lights','night_combined_lights','bunker_door_spill'])){
  ...
  if(outside&&width===1280)fs.mkdirSync('qa/results/stage-c2-visuals',{recursive:true});if(outside&&width===1280)fs.writeFileSync('qa/results/stage-c2-visuals/night-combined.png',pair[1].r.canvas.toBuffer('image/png'));
 }
 const navigation=[],commands=[],serialization=[],heaps=[];
 for(const [index,h]of pair.entries()){
  const version=index?require('../package.json').version:'0.33.0';
 ...
 fs.writeFileSync('qa/results/stage-c2-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!report.passed)process.exitCode=1;
