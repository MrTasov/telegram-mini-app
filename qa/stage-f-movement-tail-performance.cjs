...
async function measure(){
 const pair=[boot(path.join(__dirname,'stage-f-base'),{beforeScripts:s=>{s.fetch=async url=>{const b=fs.readFileSync(path.join(root,url));return {ok:true,arrayBuffer:async()=>b.buffer.slice(b.byteOffset,b.byteOffset+b.byteLength)};};}}),boot(root)],rows=[];
 ...
 for(const [width,height,mode]of [[1280,800,'PC']]){
  ...
  for(let pass=0;pass<8;pass++)for(const index of (pass%2?[1,0]:[0,1]))for(let n=0;n<45;n++){
  ...
  for(const [index,h]of pair.entries()){
   ...
   rows.push({version:index?require('../package.json').version:'0.36.1',viewport:width+'x'+height,playerDistance:travel[index].player,droneDistance:travel[index].drone,...stats(samples[index])});
   h.E("GameActions.dispatch('MOVE',{kind:'vector',x:0,y:0,power:0})");
  }
 }
 const errors=pair.flatMap(h=>h.r.errors);return {passed:rows.length===2&&!errors.length,rows,errors,scope:'Actual MOVE, update, following Drone, flashlight and Drone light, audio, native Canvas2D; 240 measured frames per version/viewport, alternating AB/BA. Distances cover measured frames only; no OS/browser input delivery.'};
}
measure().then(m=>{const runtimeSha256=require('node:crypto').createHash('sha256').update(fs.readFileSync(path.join(root,'js/game.js'))).digest('hex');const report={version:'0.37.0',runtimeSha256,purpose:'Targeted larger-sample repeat of the observed PC movement p95 increase. Original comparison is retained unchanged.',...m};fs.writeFileSync(path.join(root,'qa/results/stage-f-movement-tail-performance.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!m.passed)process.exitCode=1;}).catch(e=>{console.error(e);process.exitCode=1;});
