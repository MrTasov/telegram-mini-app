...
async function main(){
 ...
 await check('save.full029PayloadAndRoundtripPreserved',()=>{fresh();same();pair('restoreGameProgress(decodeGameProgress(JSON.stringify(captureGameProgress())))');same();assert.equal(E('captureGameProgress().saveVersion'),6);});
