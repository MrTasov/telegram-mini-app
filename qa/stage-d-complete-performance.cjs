// Immutable delivered 0.35.2 vs Stage D Corrective. Run separately from regression suites.
...
(async()=>{
 ...
 const checkpoint='qa/results/stage-d-complete-performance-progress.json';
 const retained=process.env.STAGE_D_PERF_RESUME?JSON.parse(fs.readFileSync(checkpoint)):null;
 if(retained&&(retained.runtimeSha256!==runtimeSha256||retained.benchmarkVersion!==3))throw Error('Stale performance checkpoint');
 const pair=[boot(path.join(__dirname,'stage-d-complete-base'),{beforeScripts:s=>{s.fetch=async url=>{const b=fs.readFileSync(path.join(root,url));return {ok:true,arrayBuffer:async()=>b.buffer.slice(b.byteOffset,b.byteOffset+b.byteLength)};};}}),boot(root)],rows=retained?.rows||[];
 ...
 for(const [index,h]of pair.entries()){
  const version=index?require('../package.json').version:'0.35.2';
 ...
 for(const [index,h] of pair.entries()){
   ...
   const times=[],poll=h.E(index?'()=>GameBaseControlUI.tick()':'()=>v09RefreshPowerUI()');
   for(let n=0;n<160;n++){const at=performance.now();poll();if(n>=40)times.push(performance.now()-at);}
   controlUI.push({version:index?require('../package.json').version:'0.35.2',scope:'operational Remote refresh',...stats(times)});
 ...
 report.movement=await require('./stage-d-complete-movement-performance.cjs').measure();report.passed=report.passed&&report.movement.passed;
 fs.writeFileSync('qa/results/stage-d-complete-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!report.passed)process.exitCode=1;
