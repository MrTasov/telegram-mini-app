...
const result={reference:'0.22.0 manually accepted',candidate:require('../package.json').version,ignoredFields:['gameVersion (release metadata)','no gameplay fields normalized'],passed:checks.filter(c=>c.status==='PASS').length,failed:checks.filter(c=>c.status!=='PASS').length,checks};
