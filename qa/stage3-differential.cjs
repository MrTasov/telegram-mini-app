...
const {setup}=require('./runtime.cjs'),a=setup('qa/stage2/index.html'),b=setup('index.html'),checks=[];
...
// Exhaustive existing variants/specializations/levels, compared to executable 0.22.0.
...
const result={reference:'0.22.0 manually accepted',candidate:require('../package.json').version,ignoredFields:['gameVersion (release metadata)','Approved patch contracts: world-farm-contract.cjs and bunker-contract.cjs; current R1 migration tested separately'],passed:checks.filter(c=>c.status==='PASS').length,failed:checks.filter(c=>c.status!=='PASS').length,checks};
fs.writeFileSync(path.join(__dirname,'results/stage3-differential.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({passed:result.passed,failed:result.failed,failures:checks.filter(c=>c.status!=='PASS')}));if(result.failed)process.exitCode=1;
