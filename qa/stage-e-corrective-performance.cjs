// Immutable delivered 0.36.0 vs Stage E. Run separately from regression suites.
...
(async()=>{
 ...
 const checkpoint='qa/results/stage-e-corrective-performance-progress.json';
 ...
 const pair=[boot(path.join(__dirname,'stage-e-corrective-base'),{beforeScripts:s=>{s.fetch=async url=>{const b=fs.readFileSync(path.join(root,url));return {ok:true,arrayBuffer:async()=>b.buffer.slice(b.byteOffset,b.byteOffset+b.byteLength)};};}}),boot(root)],rows=retained?.rows||[];
 ...
 for(const [index,h]of pair.entries()){
  const version=index?require('../package.json').version:'0.36.0';
 ...
 for(const [index,h] of pair.entries()){
   ...
   controlUI.push({version:index?require('../package.json').version:'0.36.0',scope:'operational Remote refresh',...stats(times)});
 ...
 report.movement=await require('./stage-e-corrective-movement-performance.cjs').measure();report.passed=report.passed&&report.movement.passed;
 fs.writeFileSync('qa/results/stage-e-corrective-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!report.passed)process.exitCode=1;
