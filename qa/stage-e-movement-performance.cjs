...
async function measure(){
 ...
 for(const [width,height,mode]of [[390,844,'MOBILE'],[1280,800,'PC']]){
  ...
  for(const [index,h]of pair.entries()){
   ...
   rows.push({version:index?require('../package.json').version:'0.35.3',viewport:width+'x'+height,playerDistance:travel[index].player,droneDistance:travel[index].drone,...stats(samples[index])});
...
if(require.main===module)measure().then(m=>{
 const file=path.join(root,'qa/results/stage-e-performance.json'),p=JSON.parse(fs.readFileSync(file));
