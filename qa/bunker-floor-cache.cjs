// Compare the cached R1 floor with the unmodified historical drawing algorithm.
...
const rect={left:610,top:-240,right:1810,bottom:1260};
const metrics=[];
for(const zoom of [.65,1,1.4])check('floor.originalPatternAndPhase.'+zoom,()=>{
 const frames=[old,current].map(r=>{r.canvas.width=900;r.canvas.height=700;r.eval(`ctx.setTransform(1,0,0,1,0,0);ctx.clearRect(0,0,900,700);ctx.setTransform(${zoom},0,0,${zoom},${-610*zoom},${240*zoom});V011Rooms.floor('corridor',${JSON.stringify(rect)});`);return Buffer.from(r.canvas.getContext('2d').getImageData(0,0,900,700).data);});
...
check('cache.oneBufferNoNewImageOrGameplayState',()=>{const before=current.eval('JSON.stringify(captureGameProgress())'),requests=current.imageRequests.length;for(let i=0;i<20;i++)current.eval(`V011Rooms.floor('corridor',${JSON.stringify(rect)});`);assert.equal(current.eval('JSON.stringify(captureGameProgress())'),before);assert.equal(current.imageRequests.length,requests);assert.equal(current.eval('V011Rooms.floorCacheInfo().entries'),1);assert.equal(current.eval('V011Rooms.floorCacheInfo().bytes'),1186*1486*4);assert.deepEqual(current.errors,[]);});
