// Immutable delivered 0.35.1 vs Stage D Corrective. Run separately from regression suites.
...
(async()=>{
 ...
 const checkpoint='qa/results/stage-d-corrective-performance-progress.json';
 const retained=process.env.STAGE_D_PERF_RESUME?JSON.parse(fs.readFileSync(checkpoint)):null;
 if(retained&&(retained.runtimeSha256!==runtimeSha256||retained.benchmarkVersion!==2))throw Error('Stale performance checkpoint');
 const pair=[boot(path.join(__dirname,'stage-d-corrective-base'),{beforeScripts:s=>{s.fetch=async url=>{const b=fs.readFileSync(path.join(root,url));return {ok:true,arrayBuffer:async()=>b.buffer.slice(b.byteOffset,b.byteOffset+b.byteLength)};};}}),boot(root)],rows=retained?.rows||[];
 ...
 for(const [index,h]of pair.entries()){
  const version=index?require('../package.json').version:'0.35.1';
 ...
 after.E("scene='bunker';player.x=1210;player.y=675;V09Power.running=true;V09Power.fuel=100;for(const type of ['iron','parts','wood','concrete'])addItem(type,100);window.perfCraft=GamePlacement.request('furnace','craft');GamePlacementUI.show(perfCraft.instanceId)");
 ...
 const maxEquipment=[];
 // Saturate every currently available type through the real authority. Registry
 // capacity is 48; the present catalog permits 42 records, including paused farm.
 after.E(`restoreGameProgress(decodeGameProgress(${JSON.stringify(saves[1])}));menuOpen=false;GameFlow.resume();scene='bunker';player.x=1210;player.y=675;V09Power.running=true;V09Power.fuel=100;for(const d of v09Doors)d.open=1;bag=[];for(const c of storageChests)c.items=[];if(!V014Robots.pack())throw Error('Stress fixture must pack the real drone token');for(const t of ['iron','parts','wood','concrete','copper'])addItem(t,300);`);
 after.E(`window.perfPlaced=[];for(const [type,rule]of Object.entries(GamePlacement.rules)){if(!rule.craftable)continue;while(GameEquipment.records.filter(r=>r.typeId===type).length<rule.limit){const result=GamePlacement.request(type,'craft');if(!result.ok)throw Error(type+': '+result.reason);perfPlaced.push(result.instanceId);}}GameMovable.receive('base_lamp',8);for(const r of GameEquipment.inventory(GameActors.localId))if(!perfPlaced.includes(r.id))perfPlaced.push(r.id);`);
 after.E(`for(const id of perfPlaced){const type=GameEquipment.get(id).typeId;let placed=false;outer:for(const room of GamePlacement.zones){const b=BunkerLayout.rooms[room];for(const turn of [0,1,2,3])for(let y=b.top+50;y<b.bottom-30;y+=40)for(let x=b.left+40;x<b.right-30;x+=40){const t=GamePlacement.centered(type,room,x,y,turn);if(GamePlacement.check(id,t).ok){const p=GamePlacement.request(id,'place',t);if(!p.ok)throw Error('Stress placement rejected: '+p.reason);placed=true;break outer;}}}if(!placed)throw Error('No stress placement for '+id);}for(const id of GameEquipment.productionIds){const type=GameEquipment.recipeStation(id);if(type==='furnace')V09Craft.start(id,'iron',4);if(type==='craft_bench')V09Craft.start(id,'ammo',4);if(type==='utility_workbench')V09Craft.start(id,'flashlight',4);}for(const r of GameEquipment.records.filter(r=>r.typeId==='base_lamp')){V010Energy.setRoomPriority(r.transform.room,3);V010Energy.setDevicePriority(r.refs.device,3);}const dense=GamePlacement.zones.map(room=>({room,count:GameEquipment.records.filter(r=>r.placement==='installed'&&r.transform.room===room).length})).sort((a,b)=>b.count-a.count)[0];window.perfDenseRoom=dense;const b=BunkerLayout.rooms[dense.room],points=[];for(let y=b.top+40;y<b.bottom-30;y+=20)for(let x=b.left+40;x<b.right-30;x+=20)if(!worldCollision(x,y,player.radius,'bunker'))points.push({x,y,d:Math.hypot(x-(b.left+b.right)/2,y-(b.top+b.bottom)/2)});points.sort((a,b)=>a.d-b.d);Object.assign(player,points[0]);updateCamera();`);
 assert.equal(after.E("GameEquipment.records.filter(r=>r.typeId==='base_lamp'&&devicePowered(r.refs.device)).length"),8);
 const instanceCount=after.E('GameEquipment.ids.length');assert.equal(instanceCount,42);
 assert.equal(after.E("GameEquipment.records.filter(r=>r.typeId==='base_lamp'&&r.placement==='installed').length"),8);
 after.E('window.fullEquipmentSave=JSON.stringify(captureGameProgress());restoreGameProgress(decodeGameProgress(fullEquipmentSave))');
 assert.equal(after.E('GameEquipment.ids.length'),42);
 for(const [width,height]of [[390,844],[1280,800]]){after.E(`window.innerWidth=${width};window.innerHeight=${height};resizeCanvas();updateCamera()`);const times=[],frame=after.E('()=>{update();GameAudioWorld.tick();draw()}');for(let n=0;n<160;n++){after.advance(17);const at=performance.now();frame();after.r.canvas.getContext('2d').getImageData(0,0,1,1);if(n>=40)times.push(performance.now()-at);}maxEquipment.push({viewport:width+'x'+height,instances:instanceCount,placedLamps:8,poweredLamps:after.E("GameEquipment.records.filter(r=>r.typeId==='base_lamp'&&devicePowered(r.refs.device)).length"),denseRoom:after.J('perfDenseRoom'),...stats(times)});}
 const report={benchmarkVersion:2,audioBuffers:pair.map(h=>h.E('GameAudio.inspect().buffers')),resumed:!!retained,version:require('../package.json').version,runtimeSha256:require('node:crypto').createHash('sha256').update(fs.readFileSync('js/game.js')).digest('hex'),baseline:'immutable delivered 0.35.1 Stage D',environment:{node:process.version,cpu:os.cpus()[0]?.model,mode:'Modeled DOM/WebAudio, native Canvas2D, DPR 1, zoom .65, 120 measured frames per scene/version; alternating AB/BA'},rows,navigation,commands,serialization,heaps,placementCheck:stats(placement),placementPreview:stats(preview),idlePlacementChecks:0,maxEquipment,campaignPoll:{closed:stats(closed),open:stats(opened)},droneMetrics:after.J('V016Lighting.droneMetrics()'),lightingCache:after.J('V016Lighting.cacheInfo()'),errors:pair.flatMap(h=>h.r.errors),limitations:['CPU measurements on a shared host, not phone/browser FPS, GPU time or physical touch latency.','Moving-light scenes include continuously changing drone positions and player aim; night scene runs all eight searchlights.','Command dispatch excludes OS/browser delivery. Navigation covers complete path queries.','Heap deltas include harness and serialization allocations, not a browser/device memory budget.']};
 ...
 fs.writeFileSync('qa/results/stage-d-corrective-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!report.passed)process.exitCode=1;
