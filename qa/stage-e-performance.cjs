// Immutable delivered 0.35.3 vs Stage E. Run separately from regression suites.
...
(async()=>{
 ...
 const checkpoint='qa/results/stage-e-performance-progress.json';
 const retained=process.env.STAGE_E_PERF_RESUME?JSON.parse(fs.readFileSync(checkpoint)):null;
 if(retained&&(retained.runtimeSha256!==runtimeSha256||retained.benchmarkVersion!==1))throw Error('Stale performance checkpoint');
 const pair=[boot(path.join(__dirname,'stage-e-base'),{beforeScripts:s=>{s.fetch=async url=>{const b=fs.readFileSync(path.join(root,url));return {ok:true,arrayBuffer:async()=>b.buffer.slice(b.byteOffset,b.byteOffset+b.byteLength)};};}}),boot(root)],rows=retained?.rows||[];
 ...
 for(const [width,height,mode]of (process.env.STAGE_E_SHORT?[[390,844,'MOBILE']]:[[390,844,'MOBILE'],[1280,800,'PC']]))for(const sceneName of (process.env.STAGE_E_SHORT?['bunker_moving_lights','night_combined_lights','bunker_door_spill']:['quiet_surface','stress_surface','bunker_hall','bunker_moving_lights','night_combined_lights','bunker_door_spill'])){
  ...
  if(outside&&width===1280)fs.mkdirSync('qa/results/stage-e-visuals',{recursive:true});if(outside&&width===1280)fs.writeFileSync('qa/results/stage-e-visuals/night-combined.png',pair[1].r.canvas.toBuffer('image/png'));
 }
 const navigation=[],commands=[],serialization=[],heaps=[];
 for(const [index,h]of pair.entries()){
  const version=index?require('../package.json').version:'0.35.3';
 ...
 for(const [index,h] of pair.entries()){
   ...
   controlUI.push({version:index?require('../package.json').version:'0.35.3',scope:'operational Remote refresh',...stats(times)});
 ...
 report.movement=await require('./stage-e-movement-performance.cjs').measure();report.passed=report.passed&&report.movement.passed;
 fs.writeFileSync('qa/results/stage-e-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!report.passed)process.exitCode=1;
