// Immutable delivered 0.32.1 vs current Stage C1. Run separately from regression suites.
...
(async()=>{
 const pair=[boot(path.join(__dirname,'stage-c1-base')),boot(root)],rows=[];
 ...
 for(const [width,height,mode]of (process.env.STAGE_C1_PREREQ?[[390,844,'MOBILE']]:[[390,844,'MOBILE'],[1280,800,'PC']]))for(const sceneName of (process.env.STAGE_C1_PREREQ?['bunker_moving_lights','night_combined_lights','bunker_door_spill']:['quiet_surface','stress_surface','bunker_hall','bunker_moving_lights','night_combined_lights','bunker_door_spill'])){
  ...
  for(const [index,h]of pair.entries()){
   ...
   if(moving){
    if(index===1)h.E("equipment.head={type:'head_mount',qty:1};addItem('flashlight',1);var qaBenchLight=bag.find(i=>i?.type==='flashlight');V010Combat.ensure(qaBenchLight);GameHeadModules.request('install',{moduleId:qaBenchLight.uid});addItem('flashlight',1);");
  ...
  for(let pass=0;pass<4;pass++)for(const index of (pass%2?[1,0]:[0,1])){
   ...
   for(let n=0;n<45;n++){h.advance(16.667);const at=performance.now();frames[index](pass*45+n);h.r.canvas.getContext('2d').getImageData(0,0,1,1);if(n>=15)samples[index].push(performance.now()-at);}
  }
  ...
  if(outside&&width===1280)fs.mkdirSync('qa/results/stage-c1-visuals',{recursive:true});if(outside&&width===1280)fs.writeFileSync('qa/results/stage-c1-visuals/night-combined.png',pair[1].r.canvas.toBuffer('image/png'));
 }
 const navigation=[],commands=[],serialization=[],heaps=[];
 for(const [index,h]of pair.entries()){
  const version=index?require('../package.json').version:'0.32.1';
 ...
 const after=pair[1];after.E("stopControls(true);scene='bunker';player.x=1210;player.y=675;V09Power.running=true;GameCampaign.refresh(true);");
 ...
 const report={version:require('../package.json').version,runtimeSha256:require('node:crypto').createHash('sha256').update(fs.readFileSync('js/game.js')).digest('hex'),baseline:'immutable accepted 0.32.1 A/B Corrective',environment:{node:process.version,cpu:os.cpus()[0]?.model,mode:'Modeled DOM/WebAudio, native Canvas2D, DPR 1, zoom .65, 120 measured frames per scene/version; alternating AB/BA'},rows,navigation,commands,serialization,heaps,campaignPoll:{closed:stats(closed),open:stats(opened)},droneMetrics:after.J('V016Lighting.droneMetrics()'),lightingCache:after.J('V016Lighting.cacheInfo()'),errors:pair.flatMap(h=>h.r.errors),limitations:['CPU measurements on a shared host, not phone/browser FPS, GPU time or physical touch latency.','Moving-light scenes include continuously changing drone positions and player aim; night scene runs all eight searchlights.','Command dispatch excludes OS/browser delivery. Navigation covers complete path queries.','Heap deltas include harness and serialization allocations, not a browser/device memory budget.']};
 report.passed=!report.errors.length&&navigation.every(n=>n.reachable===80)&&report.droneMetrics.rayCasts===0&&report.lightingCache.droneBytes<=1048576;
 fs.writeFileSync('qa/results/stage-c1-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!report.passed)process.exitCode=1;
