...
const pair=[setup('qa/stage-e-corrective-base/index.html'),setup('index.html')];
...
const report={version:require('../package.json').version,baseline:'immutable 0.36.0 Stage E',runtimeSha256:crypto.createHash('sha256').update(fs.readFileSync('js/game.js')).digest('hex'),rows,errors:pair.flatMap(r=>r.errors),limitations:['Modeled DOM timing; no actual CSS layout, browser GPU or phone FPS measured.','120 measured polls per version/case, warmed interleaved AB/BA on shared host.']};report.passed=!report.errors.length&&rows.length===12;fs.writeFileSync('qa/results/stage-e-corrective-ui-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!report.passed)process.exitCode=1;
