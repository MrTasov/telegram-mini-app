// Real Canvas2D, delivered R1 vs R2. Run separately from the suite.
...
(async()=>{
 ...
 const pair=[boot(path.join(__dirname,'bunker-r1-base')),boot(root)],rows=[];
 ...
 for(const [width,height,mode]of [[390,844,'MOBILE'],[1280,800,'PC']])for(const name of ['quiet_surface','bunker_hall','bunker_energy']){
  ...
  for(const [index,h]of pair.entries()){
   ...
   if(name==='bunker_hall')h.E(`scene='bunker';player.x=${1210};player.y=790;V09Power.running=true;V09Power.fuel=100;`);
   if(name==='bunker_energy')h.E(`scene='bunker';player.x=${index?1870:2070};player.y=${90};V09Power.running=true;V09Power.fuel=100;Object.assign(V014Robots.state,V014Robots.dockPosition(),{scene:'bunker',packed:false,hp:100,battery:25,task:'docked'});`);
   h.E('updateCamera();');
  }
 ...
 if(!prior)for(const [x,y]of [[600,30],[590,510],[650,1050],[1870,90],[1870,510],[1870,1010],[1260,1460],[2034,195]]){
  const samples=[];let reachable=0;for(let n=0;n<20;n++){const at=performance.now();const p=after.E(`findWalkPath(1120,-120,{id:'ground',kind:'ground',x:${x+n*.07},y:${y},r:0,range:1},'bunker',15)`);samples.push(performance.now()-at);if(p?.length)reachable++;}nav.push({to:{x,y},reachable,...stats(samples)});
 }
 const report={patch:'bunker-level1-rework-r2',environment:{node:process.version,cpu:os.cpus()[0]?.model,dpr:1,zoom:.65,mode:'Alternating warmed update + audio observer + real Canvas2D draw, 180 measured frames per scene/version'},baseline:'Immutable delivered LAST_BASE_0.30.0_Bunker_Level_1_Rework_R1',reusedUnchangedSurfaceAndNavigation:!!prior,rows,navigation:nav,errors:pair.flatMap(h=>h.r.errors),floorCache:after.J('V011Rooms.floorCacheInfo()'),additionalImage:{path:'assets/images/props/command_core_r2.png',compressedBytes:fs.statSync('assets/images/props/command_core_r2.png').size,width:1421,height:1107,decodedRgbaBytes:1421*1107*4},limitations:['Modeled DOM/WebAudio; no physical browser, WebView, phone GPU or subjective audio measurement.','Room cameras compare corresponding relocated rooms. Both halls include their respective Command Core sprites. Shared-host p95 includes scheduling noise.','Navigation queries use open doors; closed-door behavior is covered separately by player and drone integration tests.']};
