...
const badSaves=[
 ['future',d=>{d.saveVersion=2;}],['negative',d=>{d.saveVersion=-1;}],['fractional',d=>{d.saveVersion=1.5;}],['stringVersion',d=>{d.saveVersion='1';}],['nullVersion',d=>{d.saveVersion=null;}],
...
check('restore.subsequentAutosaveWorksOnce',()=>{
 ...
 assert.equal(writes,1);assert.equal(JSON.parse(r.storage.get('survival_base_v09_slot_1')).saveVersion,1);
});
...
check('slots.importAllocatesFreeSlotAndRetainsSource',()=>{
 ...
 assert.equal(s.eval('v09ActiveSlot'),2);assert.equal(s.eval('V014Robots.state.packed'),true);assert.ok(s.storage.has('survival_base_v09_slot_1'));assert.equal(JSON.parse(s.storage.get('survival_base_v09_slot_2')).saveVersion,1);
});
...
check('slots.currentFormatSurvivesFullRestart',()=>{
 ...
 const raw=s.storage.get('survival_base_v09_slot_1');assert.equal(JSON.parse(raw).saveVersion,1);
