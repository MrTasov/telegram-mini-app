...
check('registry.historicalOrderIsExplicit',()=>assert.deepEqual(copy(E('GameSave.describe()')),require('./save-adapters.json').order));
...
check('format.explicitVersion',()=>assert.equal(snap().saveVersion,3));
...
check('format.legacyMigratesWithoutChangingInput',()=>{const before=JSON.stringify(fixture);restore(fixture);assert.equal(JSON.stringify(fixture),before);assert.equal(snap().saveVersion,3);});
...
const badSaves=[
 ['future',d=>{d.saveVersion=3;}],['negative',d=>{d.saveVersion=-1;}],['fractional',d=>{d.saveVersion=1.5;}],['stringVersion',d=>{d.saveVersion='1';}],['nullVersion',d=>{d.saveVersion=null;}],
...
check('restore.subsequentAutosaveWorksOnce',()=>{
 ...
 assert.equal(writes,1);assert.equal(JSON.parse(r.storage.get('survival_base_v09_slot_1')).saveVersion,3);
});
...
check('slots.importAllocatesFreeSlotAndRetainsSource',()=>{
 ...
 assert.equal(s.eval('v09ActiveSlot'),2);assert.equal(s.eval('V014Robots.state.packed'),true);assert.ok(s.storage.has('survival_base_v09_slot_1'));assert.equal(JSON.parse(s.storage.get('survival_base_v09_slot_2')).saveVersion,3);
});
...
check('slots.currentFormatSurvivesFullRestart',()=>{
 ...
 const raw=s.storage.get('survival_base_v09_slot_1');assert.equal(JSON.parse(raw).saveVersion,3);
