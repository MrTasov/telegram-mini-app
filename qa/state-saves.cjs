...
check('registry.historicalOrderIsExplicit',()=>assert.deepEqual(copy(E('GameSave.describe()')),Object.fromEntries(Object.entries(require('./save-adapters.json').order).map(([k,v])=>[k,[...v,'bunker.level1','campaign.foundation','equipment.instances','inventory.head-modules','base.recovery','campaign.chapter-one','equipment.placement','inventory.buildables','base.control','research.foundation','world.exploration','story.archive','defense.foundation',...(k==='decode'?[]:['simulation.activity']),'dev.qa']]))));
