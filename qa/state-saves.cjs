...
check('format.explicitVersion',()=>assert.equal(snap().saveVersion,2));
...
check('format.legacyMigratesWithoutChangingInput',()=>{const before=JSON.stringify(fixture);restore(fixture);assert.equal(JSON.stringify(fixture),before);assert.equal(snap().saveVersion,2);});
...
check('restore.subsequentAutosaveWorksOnce',()=>{
 ...
 assert.equal(writes,1);assert.equal(JSON.parse(r.storage.get('survival_base_v09_slot_1')).saveVersion,2);
});
...
check('slots.importAllocatesFreeSlotAndRetainsSource',()=>{
 ...
 assert.equal(s.eval('v09ActiveSlot'),2);assert.equal(s.eval('V014Robots.state.packed'),true);assert.ok(s.storage.has('survival_base_v09_slot_1'));assert.equal(JSON.parse(s.storage.get('survival_base_v09_slot_2')).saveVersion,2);
});
...
check('slots.currentFormatSurvivesFullRestart',()=>{
 ...
 const raw=s.storage.get('survival_base_v09_slot_1');assert.equal(JSON.parse(raw).saveVersion,2);
