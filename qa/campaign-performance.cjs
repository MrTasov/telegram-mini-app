...
(async()=>{
 const pair=[boot(path.join(__dirname,'stage-a-base')),boot(root)],rows=[];
 ...
 for(const [index,h]of pair.entries()){
  ...
  navigation.push({version:index?'A':'R2',reachable,...stats(nav)});
  const inputFn=h.E("()=>GameActions.dispatch('MOVE',{kind:'vector',x:1,y:0,power:.4})");for(let n=0;n<500;n++){const at=performance.now();inputFn();if(n>=100)input.push(performance.now()-at);}commands.push({version:index?'A':'R2',...stats(input)});
  ...
  serialization.push({version:index?'A':'R2',characters:raw.length,utf8Bytes:Buffer.byteLength(raw),...stats(times)});heaps.push({version:index?'A':'R2',snapshots:100,transientHeapDeltaBytes:transientHeap,postGcHeapDeltaBytes:retainedHeap,gcAvailable:!!global.gc});
 }
 ...
 const report={version:'0.31.0',baseline:'immutable accepted R2 0.30.1',environment:{node:process.version,cpu:os.cpus()[0]?.model,mode:'Modeled DOM/WebAudio, native Canvas2D, DPR 1, zoom .65, 120 measured frames per scene/version'},rows,navigation,commands,serialization,heaps,campaignPoll:{closed:stats(closed),open:stats(opened)},errors:pair.flatMap(h=>h.r.errors),limitations:['These are CPU measurements on a shared host, not phone/browser FPS, GPU time or physical touch latency.','Command dispatch timings exclude OS/browser event delivery. Navigation timings are complete path queries; existing incremental routing is covered by regression tests.','Heap deltas include harness and serialization allocations; post-GC values are not a browser/device memory budget.']};
 fs.writeFileSync('qa/results/campaign-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(report.errors.length||navigation.some(n=>n.reachable!==80))process.exitCode=1;
