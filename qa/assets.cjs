...
async function main(){
 await check('catalog.valid',()=>assert.equal(validate(catalog).images,134));
 ...
 await check('lazy.noDuplicateCanvasRequests',()=>{assert.equal(r.imageRequests.length,134);assert.equal(new Set(r.imageRequests).size,134);});
