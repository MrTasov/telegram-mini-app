...
async function main(){
 await check('catalog.valid',()=>assert.equal(validate(catalog).images,156));
 ...
 await check('lazy.noDuplicateCanvasRequests',()=>{assert.equal(r.imageRequests.length,156);assert.equal(new Set(r.imageRequests).size,156);});
