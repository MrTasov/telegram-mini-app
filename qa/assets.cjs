...
async function main(){
 await check('catalog.valid',()=>assert.equal(validate(catalog).images,139));
 ...
 await check('lazy.noDuplicateCanvasRequests',()=>{assert.equal(r.imageRequests.length,139);assert.equal(new Set(r.imageRequests).size,139);});
