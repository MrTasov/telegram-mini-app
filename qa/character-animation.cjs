...
async function main(){
 ...
 await check('tools.sharedCarryBodyAndTwelveStrikePhases',()=>{
  E('player.moving=false;chopState={id:worldTrees[0].id,elapsed:0,duration:2300};');const frames=[];let elapsed=1;
  for(const duration of cfg.modular.items.axe.action.durations){E(`chopState.elapsed=${elapsed/cfg.modular.items.axe.action.duration*2300}`);frames.push(E('ActorVisuals.pose("axe").frame'));elapsed+=duration;}
