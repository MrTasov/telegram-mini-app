// Immutable delivered 0.32.0 vs 0.32.1. Run separately from regression suites.
...
(async()=>{
 const pair=[boot(path.join(__dirname,'stage-ab-base')),boot(root)],rows=[];
 ...
 for(const [width,height,mode]of [[390,844,'MOBILE'],[1280,800,'PC']])for(const sceneName of ['quiet_surface','stress_surface','bunker_hall','bunker_moving_lights','night_combined_lights']){
  ...
  if(outside&&width===1280)fs.writeFileSync('qa/results/stage-ab-visuals/night-combined.png',pair[1].r.canvas.toBuffer('image/png'));
 }
 const navigation=[],commands=[],serialization=[],heaps=[];
 for(const [index,h]of pair.entries()){
  const version=index?'0.32.1':'0.32.0';
 ...
 fs.writeFileSync('qa/results/stage-ab-performance.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(!report.passed)process.exitCode=1;
