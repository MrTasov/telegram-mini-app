> Current patch: **walk-soft-steps-1**. Shared sprite walk cycle ×3, softer floor footsteps at half the previous channel gain. Movement speed and gameplay unchanged. [Report](WALK_SOFT_STEPS_RU.md).
...
Development: `npm ci`, `npm run build`, `npm run check`, `npm test`. Current before/after performance: `node qa/polish-bench.cjs` (run separately from tests). Optional parallel test workers: `LAST_BASE_TEST_JOBS=2 npm test`. JavaScript cache tag: `0.29.0-walk-soft-steps-1`. Runtime assets are included; `tools/pack-actors.cjs` is an optional offline packer requiring the original generated PNG files named in `tools/actor-sources.json`. Full ImageGen prompts are in `docs/CHARACTER_ASSET_PROMPTS.json`.
