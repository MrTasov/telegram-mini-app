> Current patch: **visual-audio-polish-1**. Gathering animation ×2 with unchanged resource timing/yield, synchronized footstep/mining audio, muzzle VFX, new ore icons, movement scale correction, current corpses ×1.2. Game/save versions remain 0.29.0 / 3. See [POLISH_RU.md](POLISH_RU.md), [POLISH_CHANGESET.json](POLISH_CHANGESET.json), and `qa/results/summary.json`. Earlier reports below describe preceding patches.
...
Development: `npm ci`, `npm run build`, `npm run check`, `npm test`. Current before/after performance: `node qa/polish-bench.cjs` (run separately from tests). Optional parallel test workers: `LAST_BASE_TEST_JOBS=2 npm test`. JavaScript cache tag: `0.29.0-visual-audio-polish-1`. Runtime assets are included; `tools/pack-actors.cjs` is an optional offline packer requiring the original generated PNG files named in `tools/actor-sources.json`. Full ImageGen prompts are in `docs/CHARACTER_ASSET_PROMPTS.json`.
