...
const {setup}=require('./runtime.cjs'),root=path.resolve(__dirname,'..');process.chdir(path.join(root,'baseline_0.20.0'));
...
async function main(){
 const before=process.memoryUsage(),boot=performance.now(),r=setup('index.html'),bootMs=performance.now()-boot,E=s=>r.eval(s);
 ...
 for(const c of cases){
  ...
  for(let i=0;i<60;i++){r.advance(16.667);E('frameScale=1;update();draw();');r.canvas.getContext('2d').getImageData(0,0,1,1);}
  ...
  for(let i=0;i<180;i++){r.advance(16.667);const a=performance.now();E('frameScale=1;update();');const b=performance.now();E('draw();');r.canvas.getContext('2d').getImageData(0,0,1,1);const end=performance.now();update.push(b-a);draw.push(end-b);frames.push(end-a);}
