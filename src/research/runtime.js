...
window.GameResearch=(()=>{
  let sequence=0;
  function facts(){const result={'base.core_powered':devicePowered(GameCampaign.powerId),'base.drone_station':GameEquipment.records.some(r=>r.typeId==='drone_station'&&GameEquipment.present(r.id)),'campaign.chapter_one_complete':GameCampaign.view().completed.includes('chapter_1')};for(const id of Object.keys(V010Progression.TECH))result['legacy.'+id]=V010Progression.isLegacyUnlocked(id);return result;}
  const domain=ResearchDomain.create(ResearchDefinitions,{coreId:BunkerLayout.core.id,initialProfile:'legacy',facts,actor:id=>GameActors.get(id),authorized:a=>a.id===GameActors.localId,access:(a,id)=>GameCampaign.access(a,id,true),changed(){queueGameSave();}});
  ...
  function request(action,payload,revision=domain.revision){return execute({actorId:GameActors.localId,instanceId:BunkerLayout.core.id,action,payload,expectedRevision:revision,requestId:'research:'+revision+':'+(++sequence)});}
  function migrate(d){if(d.research036)throw Error('Unexpected research owner in old save');d.research036=domain.fresh('legacy');}
  function validate(d){if(!d.research036)throw Error('Missing research owner');const f={'campaign.chapter_one_complete':d.campaign031?.completed?.includes('chapter_1')===true};for(const id of Object.keys(V010Progression.TECH))f['legacy.'+id]=d.v010?.modules?.progression?.unlocks?.includes(id)===true;return domain.validate(d.research036,f);}
  ...
  GameSave.extend('restore','research.foundation',function(previous,d){validate(d);const out=previous(d);domain.restore(d.research036);return out;});
