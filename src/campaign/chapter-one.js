...
window.GameChapterOne=(()=>{
 ...
 function confirmRepair(id){
   if(!active()||!ChapterOnePreset.criticalRepairs.includes(id)||state.repaired.includes(id))return false;
   const o=V018Build.record(id)?.object;if(!o||o.hp<o.maxHp)return false;
   state.repaired.push(id);queueGameSave();return true;
 }
 function reconcileRepairs(){if(!active()||GameSave.restoring)return;for(const id of ChapterOnePreset.criticalRepairs)confirmRepair(id);}
 ...
 function facts(){reconcileRepairs();return {c1Tools:state.seen.tools,c1Fuel:state.seen.fuel,c1Tank:state.seen.tank,c1Power:state.seen.power,c1Core:state.seen.core,c1Concrete:V010Progression.state.byResource.produced.concrete||0,c1Repairs:state.repaired.length,c1Night:state.nightSurvived};}
 function validate(d){
 ...
 return Object.freeze({fresh,migrate,validate,observe,visit,facts,onDeath,confirmRepair,reconcileRepairs,get active(){return active();}});
})();
