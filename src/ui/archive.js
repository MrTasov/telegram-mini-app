...
window.GameArchiveUI=(()=>{
  ...
  CommandCoreUI.registerSection('archive','story.archive',(host,route)=>{
    ...
    function render(){
      ...
      for(const e of visible){const card=node('section',null,'archiveCard');card.dataset.archiveEntry=e.id;const title=node('h4',e.title),summary=node('p',e.summary),read=node('small',view.read.includes(e.id)?'story.read':'story.unread','archiveReadState'),action=button('story.open',()=>open(e.id));action.setAttribute('aria-label',t('story.open')+': '+t(e.title));card.append(title,summary,read,action);host.append(card);}
      host.scrollTop=top;
    }
    return render;
  ...
  v09Style(`#coreSection_archive .archiveFilter{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin:10px 0 14px;font-size:12px}#coreSection_archive .archiveFilter select{flex:1;min-height:44px;min-width:130px}#coreSection_archive .archiveCard{border:1px solid #45625b;border-radius:9px;padding:12px;margin:8px 0;background:#192e31}#coreSection_archive .archiveCard h4{font-size:15px;margin:0 0 6px;color:#e5eddc}#coreSection_archive .archiveCard p{font-size:12px;color:#bbcec3;margin:0 0 8px}#coreSection_archive .archiveCard button{display:block;width:100%;min-height:44px;margin:8px 0 0}#coreSection_archive .archiveReadState{display:block;color:#b6c7ab;font-size:11px;margin:6px 0}#coreSection_archive .archiveDocument p{font-size:15px;line-height:1.65;white-space:pre-line;color:#d6e2d8}#coreSection_archive .archiveDocument img{max-width:100%;height:auto;border-radius:8px}#coreSection_archive .archiveDocument button{min-height:44px}`);
