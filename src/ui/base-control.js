    ...
    function refresh(){
      ...
      if(sig!==built){built=sig;cards.clear();levels.replaceChildren();map.replaceChildren();rows.replaceChildren();
        ...
        const counts=new Map();for(const d of devices){counts.set(d.name,(counts.get(d.name)||0)+1);const row=node('section','controlObject'),text=node('div','controlObjectText'),name=node('strong'),status=node('small'),actions=node('div','controlObjectActions');row.dataset.controlInstance=d.id;name.textContent=(d.kind==='door'?d.name+' · '+t('autoOpen'):d.name)+(devices.filter(x=>x.name===d.name).length>1?' · '+counts.get(d.name):'');text.append(name,status);row.append(text,actions);rows.append(row);
          ...
          let auto=null,light=null;if(d.kind!=='drone'){main.classList.add('controlSwitch');main.setAttribute('role','switch');}
      ...
      for(const d of devices){const c=cards.get(d.id);if(!c)continue;const powered=d.deviceId?a.served.has(d.deviceId):true,key=d.broken?'damaged':!d.on?'off':d.kind==='door'?powered?'automatic':'noPower':d.kind==='gate'?'opened':!powered?'noPower':'on';
        ...
        if(c.auto){put(c.auto,t('auto'));c.auto.classList.toggle('selected',d.mode==='auto');}if(c.light){put(c.light,t(d.light?'droneLightOff':'droneLightOn'));c.light.setAttribute('aria-pressed',String(d.light));c.light.setAttribute('aria-label',t(d.light?'droneLightOff':'droneLightOn'));}
      }
...
.controlObjectActions .controlSwitch{position:relative;width:48px!important;min-width:48px!important;height:44px!important;min-height:44px!important;padding:0!important;font-size:0!important;border:0!important;background:transparent!important}.controlSwitch:before{content:'';position:absolute;inset:9px 2px;border:1px solid #80988a;border-radius:18px;background:#30443e}.controlSwitch:after{content:'';position:absolute;left:6px;top:13px;width:18px;height:18px;border-radius:50%;background:#bacabd}.controlSwitch.on:before{background:#387e5d;border-color:#acd0b7}.controlSwitch.on:after{transform:translateX(18px);background:#eff6dc}.controlSwitch:focus-visible{outline:2px solid #f0dd9b;outline-offset:2px}.controlSwitch:disabled{opacity:.45}.controlObjectText strong{line-height:1.5}
