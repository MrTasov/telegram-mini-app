...
window.StoryPlayer=(()=>{
  ...
  function render(){if(!current)return;const e=entry(current.id);heading.textContent=t(e.title);const cue=e.subtitles.find(c=>current.elapsed>=c.startMs&&current.elapsed<c.endMs)||e.subtitles.at(-1);subtitle.textContent=t(cue?.text||e.body[0]);toggle.textContent=t(current.playing?'story.pause':'story.play');toggle.setAttribute('aria-pressed',String(!current.playing));leave.textContent=t(current.intro?'story.skip':'story.close');notice.hidden=!current.fallback&&!current.blocked;notice.textContent=current.blocked?t('story.video.blocked'):current.fallback?t('story.fallback'):'';overlay.classList.toggle('storyHasVideo',!!media&&e.media?.kind==='video');overlay.querySelector('.v09Panel').setAttribute('aria-label',t(e.title));overlay.querySelector('.v09Close').setAttribute('aria-label',t(current.intro?'story.skip':'story.close'));const image=mediaHost.querySelector('img');if(image&&e.media)image.alt=t(e.media.alt);}
  const originalClose=closeOverlay;
  function cleanup(){generation++;releaseMedia();current=null;overlay.classList.remove('storyHasVideo');originalClose(overlay);GameFlow.resume('story:local');stopControls(true);const focus=previousFocus;previousFocus=null;focus?.focus?.();}
