...
window.GameStory=(()=>{
  ...
  function savedFacts(d){return {'story.context':true,'story.core_visited':d.chapter034?.seen?.core===true,'story.research_count':d.research036?.completed?.length||0,'story.chapter_one_complete':d.campaign031?.completed?.includes('chapter_1')===true};}
  function facts(){if(researchEpoch!==GameResearch.epoch){researchEpoch=GameResearch.epoch;researchCount=GameResearch.capture().completed.length;}return {'story.context':true,'story.core_visited':GameChapterOne.facts().c1Core===true,'story.research_count':researchCount,'story.chapter_one_complete':GameCampaign.view().completed.includes('chapter_1')};}
