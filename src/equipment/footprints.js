...
window.GameFootprints=(()=>{
  ...
  function fit(f,spec){
    const [key,dx,dy,w,h]=spec,meta=AssetManifest.images[GameAssets.artId(key)],b=meta.crop||{w:meta.size[0],h:meta.size[1]},scale=Math.min(w/b.w,h/b.h);
  ...
  function body(id){
    ...
    else if(r&&art[r.typeId]){const local=fit({id,x:0,y:0,room:f.room},art[r.typeId]);b={id,room:f.room,...EquipmentInstances.aabb(r.transform,local)};}
  ...
  function forRecord(r){const spec=art[r.typeId],def=EquipmentInstances.definitions[r.typeId];const local=spec?fit({id:r.id,x:0,y:0,room:r.transform.room},spec):{x:0,y:0,...def.footprint};return {id:r.id,room:r.transform.room,...EquipmentInstances.aabb(r.transform,local)};}
  return Object.freeze({body,forRecord});
})();
