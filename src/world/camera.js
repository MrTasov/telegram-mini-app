...
const V010Camera=(()=>{
  let zoom=.8,targetZoom=.8,tilt=1,opacity=0,corner='right',mapOpen=false,clock=0,selected=null,mapScene=null;
  ...
  function reveal(actor=GameActors.local){if(actor.scene!=='surface'||actor.dead)return;const observer=actor.entity;const cx=Math.floor((observer.x+1000)/CELL),cy=Math.floor(observer.y/CELL);for(let dx=-2;dx<=2;dx++)for(let dy=-2;dy<=2;dy++){const x=cx+dx,y=cy+dy;if(dx*dx+dy*dy<=6&&x>=-12&&x<=33&&y>=-15&&y<60)discovered.add(x+','+y);}
    const places=window.V010World?.places||[];for(const p of places){if(known.has(p.id)||Math.hypot(p.x+p.w/2-observer.x,p.y+p.h/2-observer.y)>350)continue;known.add(p.id);V010.emit('discover',{id:p.id,kind:p.zone||'place'});}
    for(const p of V09World.ores)if(!known.has(p.id)&&Math.hypot(p.x-observer.x,p.y-observer.y)<380){known.add(p.id);V010.emit('discover',{id:p.id,kind:'ore'});}
    for(const r of window.V010World?.regions||[])if(!known.has(r.id)&&Math.hypot(r.x-observer.x,r.y-observer.y)<r.r*.5){known.add(r.id);V010.emit('discover',{id:r.id,kind:'region'});}
  }
  ...
  function mapDraw(c,w,h,miniMode){
    const which=miniMode?scene:(mapScene||scene);
    c.clearRect(0,0,w,h);c.save();if(miniMode){c.beginPath();c.arc(w/2,h/2,Math.min(w,h)/2,0,Math.PI*2);c.clip();}c.fillStyle='#13221d';c.fillRect(0,0,w,h);const b=bounds(which);
    const scale=miniMode?(which==='surface'?.105:.13):Math.min(w/b.w,h/b.h)*mapZoom;
    const markerScale=miniMode?scale/1.35:scale;
    ...
    c.fillStyle='#344536';c.fillRect(b.x,b.y,b.w,b.h);if(which==='surface')window.V012Map?.drawRoads(c);
    if(which==='surface'){
    ...
    for(const m of markers){if(m.scene!==which)continue;circle(c,m.x,m.y,Math.max(22,3/markerScale),'#ddb979');if(!miniMode){c.fillStyle='#e6dbba';c.font=`${11/scale}px Arial`;c.textAlign='center';c.fillText(m.name,m.x,m.y-30);}}
    if(goal?.scene===which){c.strokeStyle='#e9c981';c.lineWidth=2/scale;c.beginPath();c.arc(goal.x,goal.y,Math.max(45,6/scale),0,Math.PI*2);c.stroke();}
    if(which===scene){circle(c,player.x,player.y,Math.max(26,3.6/markerScale),'#f1ead1');c.strokeStyle='#f1ead1';c.lineWidth=Math.max(14,1.6/markerScale);c.beginPath();c.moveTo(player.x,player.y);const aimLength=Math.max(65,9/markerScale);c.lineTo(player.x+player.aimX*aimLength,player.y+player.aimY*aimLength);c.stroke();}
    window.V0151Station?.drawMap(c,markerScale,miniMode,which);
    ...
    if(miniMode){c.fillStyle='#d6e2d6';c.font='12px Arial';c.textAlign='center';c.fillText(I18n.text(which==='surface'?'N ↑':'Бункер'),w/2,15);if(goal?.scene===which){const dx=(goal.x-player.x)*scale,dy=(goal.y-player.y)*scale,d=Math.hypot(dx,dy),k=Math.min(1,(Math.min(w,h)/2-16)/Math.max(1,d));if(k<1){const x=w/2+dx*k,y=h/2+dy*k,a=Math.atan2(dy,dx);c.save();c.translate(x,y);c.rotate(a);c.fillStyle='#ebc87a';c.beginPath();c.moveTo(7,0);c.lineTo(-4,-4);c.lineTo(-4,4);c.closePath();c.fill();c.restore();}c.textAlign='center';c.fillText(I18n.text(Math.round(d/scale)+' м'),w/2,h-11);}}
  ...
  function mapPoint(e){const r=full.getBoundingClientRect(),b=bounds(mapScene||scene),s=Math.min(full.width/b.w,full.height/b.h)*mapZoom;return {scene:mapScene||scene,x:((e.clientX-r.left)*full.width/Math.max(1,r.width)-full.width/2)/s+mapPan.x,y:((e.clientY-r.top)*full.height/Math.max(1,r.height)-full.height/2)/s+mapPan.y};}
  ...
  el('v010MapMark').onclick=()=>{if(!selected)return;if(markers.length>=40){message('Можно поставить до 40 меток');return;}markers.push({...selected,scene:mapScene||scene,name:(el('v010MarkerName').value.trim()||'Метка').slice(0,36)});queueGameSave();drawFull();};
  el('v010MapGoal').onclick=()=>{goal=selected?{...selected,scene:mapScene||scene}:null;queueGameSave();drawFull();};
  el('v010MapDelete').onclick=()=>{if(!selected)return;let best=-1,d=200;markers.forEach((m,i)=>{const n=Math.hypot(m.x-selected.x,m.y-selected.y);if(m.scene===(mapScene||scene)&&n<d){best=i;d=n;}});if(best>=0)markers.splice(best,1);queueGameSave();drawFull();};
  ...
  const api={resetTouch(){touches.clear();pinch=null;suppressTouchUntil=0;cancelMiniHold();},get touchActive(){return touches.size>0;},point,inverse,view,setZoom,capture,restore,validate,showMap,mapDraw,drawFull,closeMap(){mapOpen=false;cancelMiniHold();},clearGoal(){goal=null;queueGameSave();drawFull();},get goal(){return goal;},get selected(){return selected?{...selected}:null;},setSelected(p){selected=p?{x:p.x,y:p.y,scene:p.scene||mapScene||scene}:null;drawFull();},setGoal(p){goal=p?{x:p.x,y:p.y,scene:p.scene||scene}:null;queueGameSave();drawFull();},mapBounds:bounds,get mapScene(){return mapScene||scene;},get mapViewport(){return {zoom:mapZoom,pan:{...mapPan}};},get zoom(){return zoom;},get targetZoom(){return targetZoom;},get tilt(){return tilt;},reveal,seen,knows:id=>known.has(id),setView(p){mapPan={x:p.x,y:p.y};mapZoom=clamp(p.zoom||mapZoom,1,6);drawFull();}};V010.register('camera',api);updateSettings();return api;
})();
