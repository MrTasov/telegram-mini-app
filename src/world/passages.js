...
window.GamePassages=(()=>{
  ...
  for(const d of v09Doors.filter(d=>BunkerLayout.roomActive(d.room)))register(d.id,{scene:'bunker',bounds:()=>d,canOpen:()=>window.GameBaseControl?.doorMode(d.id)!=='closed'&&(window.V014Robots?.planningKey()?V014Robots.motion.doorPowered(d.room):devicePowered('door_'+d.room))||d.manual||!!window.V018Build?.isBroken(d.id)});
