...
window.GameDefense=(()=>{
 ...
 let recordEpoch=-1,recordList=[];const is=r=>!!defs[r?.typeId],records=()=>{if(recordEpoch!==GameEquipment.epoch){recordEpoch=GameEquipment.epoch;recordList=GameEquipment.records.filter(is);}return recordList;},pivot=r=>GameEquipment.point(r.id,defs[r.typeId].pivot.x,defs[r.typeId].pivot.y);
 ...
 function tick(ms){if(GameFlow.paused)return;const dt=clamp(Number(ms)||0,0,100)/1000;GameEnemyIndex.refresh();tickClock+=dt;if(tickClock>=.25){tickClock=0;settle();}
 ...
 function breachTarget(z,now,stats){if(z.x<254||z.x>1346||z.y<214||z.y>986)return null;let best=null,bestDistance=260;
  for(const r of records()){if(!operational(r)||r.transform.scene!=='surface'||r.state.settings.mountWall)continue;const b=GameFootprints.forRecord(r);if(z.x<b.x-bestDistance||z.x>b.x+b.w+bestDistance||z.y<b.y-bestDistance||z.y>b.y+b.h+bestDistance)continue;const q=contactPoint(b,z.x,z.y),dist=Math.hypot(q.x-z.x,q.y-z.y);if(dist<bestDistance&&GameActivity.clear(z.x,z.y,q.x,q.y,0,r.id)){best={r,q,dist};bestDistance=dist;}}
 ...
 function validate(data){const s=data.defense039;if(!s||s.schema!==1||Object.keys(s).sort().join()!=='commands,schema')throw Error('Invalid defense state');commands.validate(s.commands);if(!data.turret016||data.turret016.guns.length)throw Error('Legacy turret must migrate into equipment');
  for(const r of data.equipment032.instances.filter(is)){const d=defs[r.typeId],v=r.state.settings;if(/^hmg016_/.test(r.id)&&Number(r.id.slice(7))>=data.turret016.nextId)throw Error('Invalid legacy defense allocation');if(Object.keys(v).sort().join()!=='ammo,angle,fallen,mountWall'||!Number.isInteger(v.ammo)||v.ammo<0||v.ammo>(d.capacity||0)||!Number.isFinite(v.angle)||Math.abs(v.angle)>Math.PI||typeof v.fallen!=='boolean'||typeof v.mountWall!=='string'||v.mountWall&&(!d.ammoType||!V015Base.byId.has(v.mountWall))||r.state.condition.maxHp!==d.hp)throw Error('Invalid defense instance state');}
  return true;
 }
