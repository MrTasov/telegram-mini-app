...
window.V016Turret=(()=>{
  ...
  function applyEnabled(id,on){const t=guns.find(g=>g.id===id);if(!t||typeof on!=='boolean')return false;t.enabled=on;changed();return true;}
  function setEnabled(t,on){return !!t&&!!window.GameBaseControl?.request(t.id,'active',{value:!!on}).ok;}
